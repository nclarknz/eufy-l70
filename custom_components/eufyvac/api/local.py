...
# Original work: Richard Mitchell, Brendan McCluskey — Apache License 2.0
...
class Message:
    ...
    def __init__(self, command, payload=None, sequence=None, encrypt=False,
                 device=None, expect_response=True, ttl=5):
        if payload is None:
            payload = b""
    ...
    def bytes(self):
        ...
        header = struct.pack(MESSAGE_PREFIX_FORMAT, MAGIC_PREFIX, self.sequence,
    ...
    def from_bytes(cls, device, data, cipher=None):
        ...
        if return_code >> 8:
            ...
            payload_data = data[header_size + struct.calcsize(">I"):
                                header_size + payload_size
                                - struct.calcsize(MESSAGE_SUFFIX_FORMAT)]
        try:
            expected_crc, suffix = struct.unpack_from(
                MESSAGE_SUFFIX_FORMAT, data,
...
class TuyaDevice:
    """Async Tuya local protocol device with persistent connection and push updates."""

    def __init__(self, device_id, host, timeout, ping_interval, update_entity_state,
                 local_key=None, port=6668, gateway_id=None, version=(3, 3)):
    ...
    def update_local_key(self, new_key: str) -> None:
        ...
        if len(new_key) != 16:
            raise InvalidKey("Local key should be a 16-character string")
        self.cipher = TuyaCipher(new_key, self.version)
    ...
    async def async_set(self, dps):
        ...
        message = Message(Message.SET_COMMAND, payload, encrypt=True, device=self,
    ...
    async def async_ping(self, ping_interval):
        ...
        if not self._backoff:
            ...
            encrypt = False if self.version < (3, 3) else True
            message = Message(Message.PING_COMMAND, sequence=0, encrypt=encrypt,
                              device=self, expect_response=False)
            self._queue.append(message)
    ...
    async def async_update_state(self, state_message, _=None):
        if (state_message is not None and state_message.payload
    ...
    async def _async_handle_message(self):
        ...
        try:
            ...
            if isinstance(e, (InvalidMessage, MessageDecodeFailed)):
                self._LOGGER.debug("Bad message from device: %s", e)
            elif isinstance(e, asyncio.IncompleteReadError):
                if self._connected:
                    self._LOGGER.debug("Incomplete read")
            elif isinstance(e, ConnectionResetError):
                self._LOGGER.debug("Connection reset: %s", e)
                await self.async_disconnect()
        else:
        ...
        try:
            ...
            if retries == 0:
                if isinstance(e, socket.error):
                    await self.async_disconnect()
                    raise ConnectionException(f"Connection to {self} failed: {e}")
                raise TuyaException(f"Failed to send data to {self}")
            ...
            try:
                ...
                if isinstance(e, TimeoutError):
                    raise ResponseTimeoutException(
                        f"Timed out waiting for response to sequence {message.sequence}")
                raise e
    ...
    async def async_send_dps124(self, method: str, data: dict) -> str | None:
        """
        Send a base64-JSON command via DPS 124.
        Returns result code ('O', 'F', 'S') or None if no echo received.
        Filters by method + timestamp to avoid stale echoes.
        """
        import base64 as _b64
        ts = round(time.time() * 1000)
        payload = json.dumps({"method": method, "data": data, "timestamp": ts},
                             separators=(",", ":"))
        cmd = _b64.b64encode(payload.encode()).decode()
        await self.async_set({"124": cmd})
        # Allow a moment for the echo to arrive via push update
        await asyncio.sleep(1.5)
        # Echo appears in DPS 124 of the state; check cached state
        echo_b64 = self._dps.get("124")
        if echo_b64:
            try:
                echo = json.loads(_b64.b64decode(echo_b64).decode())
